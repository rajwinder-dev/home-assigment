...
export const globalHandler: ErrorRequestHandler = (error, req, res, _next) => {
  ...
  if (error.code === 'P2002') {
    ...
    error = new appError('Invalid reference. Related record does not exist.', 400, 'FOREIGN_KEY_ERROR');
  ...
  } else if (error.cause?.code === 'P0001') {
    // Raw Postgres RAISE EXCEPTION, surfaced via the driver adapter wrapper
    const rawMessage: string = error.cause.message || error.message || 'Constraint violation';
    const [maybeCode, ...rest] = rawMessage.split(':');
    const isKnownCode = /^[A-Z_]+$/.test(maybeCode.trim());

    error = new appError(
      isKnownCode ? rest.join(':').trim() : rawMessage,
      400,
    );
  }

  logger.error({ err: error, reqId: res.getHeader('x-request-id') }, error.message || 'request failed');
